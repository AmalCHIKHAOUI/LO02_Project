...
public class Partie extends Observable {
    ...
    public void debutRound(int nbJoueur) { // on distribue les cartes a tout les joueurs
    	...
    	int tempNbCarte = 0;
    	for(Iterator<Joueur> it = this.joueur.iterator(); it.hasNext(); ) { //choisir le rÃ´le pour chaque joueur
			Joueur j = /*(Joueur)*/it.next();
    ...
    public void effetHunt(CarteRumeur carte, int numJoueur) {
    	...
    	if(carte.hunt == Hunt.ANGRYMOB){ // reveler la carte d'un autre joueur
    		...
    		if(action==1) {
        		...
        		System.out.println("Carte supprimee !!!") ;
        		System.out.println("----------------------------------") ;
        		System.out.println("Joueur "+numJ+", vous prenez le prochain tour et voici votre jeu actualisé : ") ;
        		System.out.println(j1.main);
    ...
    public static void main(String[] args) {
    	...
    	while (partie.gagne() == 0) { 
    		...
    		for(Iterator<Joueur> it = partie.joueur.iterator(); it.hasNext(); ) { //choisir le rÃ´le pour chaque joueur
    			Joueur j = (Joueur)it.next();
    			j.chooseRole();
